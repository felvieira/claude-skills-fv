...
**What it does:** mechanical video/audio editing via local ffmpeg scripts (stdlib Python + subprocess, no API, no upload) — cut, join, caption burn, loudness normalization, silence/scene detection, redaction (face/plate blur), aspect-ratio fit, sync, platform export presets. Enforces a strict mechanical-vs-subjective boundary: applies a named LUT or measures LUFS, never decides what's "the best highlight" or "looks cinematic."
**When to activate:** cutting/joining existing clips, burning subtitles, normalizing audio for a platform, removing silence, changing aspect ratio, redacting a face, syncing multicam, or exporting to a platform preset.
**Takeaway:** closes the gap skill 27 explicitly names as out of scope ("post-production editing — that's ffmpeg/media pipeline, not generation"). Lossless-first by default (stream-copy on keyframe cuts); `--json` is the source of truth for any measured number, never a prose summary line.
...
**What it does:** HTML single-file presentation decks on a fixed 1920×1080 stage that scales uniformly (letterbox/pillarbox, never reflow). Before building the full deck, generates 3 single-slide previews (safe preset, bold template, wildcard) — show, don't describe. 12 named style presets with distinct typography/color, PPTX import, PDF export via Playwright, Vercel deploy.
**When to activate:** any presentation/deck/slides request, converting an existing PPTX to web, or a deck that needs to run in any browser without PowerPoint/Keynote.
**Takeaway:** the fixed-stage mechanism (`engine/viewport-base.css`) is copied near-verbatim under MIT, same curation pattern as skill 64. Maintenance note: upstream has been inactive since 2026-06-23 with unaddressed issues/PRs — this kit assumes independent maintenance going forward, an explicit user decision rather than a default.

---

#### Skill 78 — Business Discovery

**What it does:** plain-language interview with a non-technical business owner (Shopify, WhatsApp, spreadsheets) that maps existing tools into a Pantry/Prep/Plate-style data map, then generates a self-contained HTML visualization (4 tabs, interactive Sankey), a prioritized opportunity list, and a 30-day setup plan. Covers 9 business archetypes (e-commerce, SaaS, professional services, healthcare clinic, wealth advisory, content creator, multi-location restaurant, real-estate brokerage, local trades). Audits any existing `.claude/` setup before asking questions, so it never re-asks what's already built.
**When to activate:** a business owner (not a dev team) wants to know what to automate first, before any code repository exists.
**Takeaway:** distinct from skill 51 (UX Research), which does user/problem discovery inside a team that already has a codebase — this skill runs *before* that, mapping tools and data for someone who may never have opened a terminal. Concept inspired by Mark Kashef's "Perfect Agentic OS Kit" (`/silver-platter`), distributed pay-what-you-want via Gumroad with no declared license — every line of prose, the 9 archetype question chains, recipe templates, and both Python scripts were written from scratch in Portuguese, in this kit's own format.

...
|---|---|---|
| **v2.76.0** | **Skill 74 nova: o elo que faltava entre asset 3D pronto e página real.** O kit tinha as duas pontas e nada no meio — as skills 68/69 certificavam um `.glb` mas todo handoff apontava pra Unity/Unreal/Godot, nunca pro browser; a skill 02 já tinha 53 guidelines verificadas de three.js num CSV consultável por BM25, mas ninguém era **dono** de "cena 3D no browser"; e "scene description" (o contrato que um LLM emite pro runtime consumir) tinha zero ocorrências no repo. A 74 fecha isso: schema de scene description versionado (câmera, luzes, ambiente, slots de GLB, materiais, partículas, interações, budget, fallback, a11y — trocar um campo muda a cena sem tocar em código), WebGPURenderer com fallback WebGL2 automático, câmera navegável com raycasting, e orçamento medido na página composta. Três achados que mudaram o desenho: WebGPU atingiu **Baseline em janeiro de 2026** (Chrome/Edge, Firefox no Windows e macOS Tahoe, Safari 26+), então WebGPU é o caminho primário e não um opt-in exótico; `setAnimationLoop()` cobre a inicialização do backend sozinho (`await renderer.init()` só antes do primeiro frame); e material embutido atravessa pro backend WebGPU, mas `ShaderMaterial`/`onBeforeCompile` **não** — precisam de TSL, e é aí que migração de projeto WebGL antigo quebra em silêncio. A fronteira com a 64 é o **input**, não o visual: se o visitante só rola, é da 64 (cuja regra dura "NAO render 3D" continua valendo no domínio dela); se ele controla a câmera, é da 74. As guidelines de código seguem no `threejs.csv` da skill 02 — referenciadas, não duplicadas, pra não criar duas fontes de verdade que envelhecem em ritmos diferentes. | [`skills/74-web3d-scene-runtime/`](skills/74-web3d-scene-runtime/) |
| **v2.75.0** | **Dashboard passa a rastrear runs do `/swarm` ao vivo, e a skill 01 ganha um gate contra o retrabalho mais caro de MVP: decisão de produto que ficou implícita.** Nova aba "Runs do /swarm" lê `stories.json`/`plan-execution.json`/`state.env` por run e mostra fase (planejando/rodando/bloqueada/concluída), progresso story a story e link do PR — diferente do `/loop` (um `status.json` único ao final), o swarm atualiza progresso em voo, então a rota re-deriva o resumo a cada chamada em vez de confiar num snapshot. As duas abas de runs agora fazem polling a cada 5s enquanto ficam abertas — testado em browser real com uma run sintética virando "concluída" sem precisar trocar de aba pra forçar reload. Separadamente, uma investigação (disparada por um post sobre por que MVPs travam na frente do primeiro cliente de verdade) confirmou que 4 de 5 decisões de produto citadas — posse do dado, permissão por ator, estados/transições de fluxo, ciclo de vida do cliente (entrada/convite/cancelamento/exportação) — não tinham checkpoint algum na spec inicial, só apareciam tecnicamente mais tarde (ex: segurança audita RBAC depois do código escrito). Nova seção "Decisões Estruturais Antes da Primeira Tela" na skill 01 força essas decisões antes do handoff pra UI/UX; a skill 11 (reviewer) ganha item de gate cobrando isso antes de aprovar deploy. | [`docs/preview/dashboard.html`](docs/preview/dashboard.html), [`scripts/dashboard-server.mjs`](scripts/dashboard-server.mjs), [`skills/01-po-feature-spec/SKILL.md`](skills/01-po-feature-spec/SKILL.md), [`skills/11-reviewer/SKILL.md`](skills/11-reviewer/SKILL.md), [`policies/prd-validation.md`](policies/prd-validation.md) |
| **v2.74.0** | **Skill nova: auditoria e desenho de funil de conversão de assinatura SaaS.** A skill 73 classifica o produto em playbook A (consumer/resultado pessoal) ou B (ferramenta B2B/PLG) e usa essa classificação para recomendar telas de onboarding, evento de ativação, paywall completo, gatilhos in-app e lifecycle de e-mail — com métricas-alvo de benchmark real (R/1K, TTV, trial-to-paid por modelo comercial), não achismo. Aprofunda taticamente a seção de monetização que a skill 01 já cobria em nível de discovery; entrega para a skill 63 (UI de checkout mobile) a decisão de modelo comercial e momento do paywall. Conteúdo consolidado a partir de material de benchmark de conversão fornecido pelo usuário. | [`skills/73-saas-conversion-playbook/`](skills/73-saas-conversion-playbook/) |
| **não lançado** | **"Loops and Graphs" absorvido — e a auditoria achou bugs no próprio kit, não só conceitos faltando.** Um subagent comparou as dez teses [do artigo](https://x.com/hanakoxbt/status/2091515787366306154) contra o código real (três já tinham enforcement de verdade; duas não valiam mecanizar). Cinco gaps eram reais. **"And then" não é aresta:** `validate-program.mjs` agora avisa quando `depends_on: [X]` nunca lê `${steps.X…}` — nada atravessa aquela aresta, então a espera é serialização pura. Achou uma real na hora: o step `security` do `comprehensive-review.yml` declarava dependência de `parallel-review` consumindo só `${inputs.pr_number}`, custando uma fase serial inteira; security virou o 6º agente dentro do bloco paralelo. Warning, não erro, porque side-effect de filesystem é dependência legítima que a sintaxe não mostra. **Retornar a unidade, não o lote:** `return-envelope.mjs` extrai UNIT/VERDICT/REASON/EVIDENCE/**SCOPE** do output de validação em vez de despejar o blob inteiro no próximo prompt — SCOPE é o que impede uma correção de uma linha virar diff de quatro arquivos, e uma falha não-parseada estreita pro nível da run em vez de alargar permissão. **`split_by`** torna a dimensão do corte de um fan-out uma decisão declarada e revisável (`lens | unit-of-work | blast-radius | source`). **Uma lane fechada** entra na permission ladder: escrita em dados de produção emite `deny` e ignora o escape hatch, porque uma lane que qualquer sufixo abre é um threshold com nome de lane. **Um check que o modelo consegue contornar não é check:** o loop não aceita mais a prosa "all tests pass" como conclusão sem os validadores determinísticos concordarem. Além disso: o `fixDestination` do circuit breaker mudou de `weak_verification` pra um novo `bad_plan` — quando a mesma correção falha N vezes, a verificação funcionou todas as vezes; quem foi mal cortada foi a unidade, e mais um teste de regressão não conserta isso. | [`scripts/auto-loop/return-envelope.mjs`](scripts/auto-loop/return-envelope.mjs), [`scripts/validate-program.mjs`](scripts/validate-program.mjs), [`hooks/scripts/permission-ladder-guard.mjs`](hooks/scripts/permission-ladder-guard.mjs), [`policies/programs-schema.md`](policies/programs-schema.md) |
| **não lançado** | **O dashboard mostra a forma do trabalho e as execuções, não só inventário.** Doze abas listavam skills, projetos e memória; nenhuma mostrava como o trabalho é estruturado nem o que aconteceu quando rodou. **Programas (grafo)** renderiza cada `programs/*.yml` como DAG — nós coloridos por tipo de step, gates como losangos, branches de `parallel` aninhados dentro do pai — onde o estilo da aresta carrega o veredito do validador: sólida significa que dado atravessa, tracejada significa só ordem sequencial, e **vermelha significa dependência declarada que nada atravessa**. A aresta falsa fica visível de imediato em vez de enterrada no YAML. **Runs do /loop** lê `.auto/runs/<id>/status.json` e tabela `stopReason`, `fixDestination`, iterações e tokens; como o `/loop` nunca rodou neste repo, ela traz um empty state que explica o que popula a aba, em vez de uma tabela vazia sem contexto. Dois bugs apareceram no teste em browser: o próprio `design-anchor-guard` do kit **bloqueou a primeira versão** — a paleta dos nós usava `#6366f1`, o indigo default do Tailwind, exatamente o sinal de "caiu na média do treino" que a regra existe pra pegar (paleta refeita derivando do accent teal já existente); e o Cytoscape renderizou o grafo espremido numa coluna porque o container mediu largura zero enquanto a aba estava oculta — `resize()` sozinho não resolve, o layout precisa rodar de novo. | [`docs/preview/dashboard.html`](docs/preview/dashboard.html), [`scripts/dashboard-server.mjs`](scripts/dashboard-server.mjs) |
| **não lançado** | **Dois bugs reais achados e corrigidos logo depois do enforcement do harness acima entrar — um por review de segurança em background, outro por auto-auditoria.** Um review de segurança em background no commit de enforcement apontou 2 bypasses reais no `hooks/scripts/permission-ladder-guard.mjs`: aspas ao redor do nome do comando (`"rm" -rf` escapa de uma regex ancorada em `\b` porque o shell trata `"rm"` e `rm` como idênticos, mas o padrão não), e esconder o comando perigoso dentro de `&&`/`;`/`$(...)` onde só o primeiro segmento era checado. Os dois confirmados manualmente antes do fix — o hook agora divide comandos compostos em cada sub-comando que um shell rodaria e normaliza aspas antes de casar, com um comentário explícito de "SECURITY MODEL" documentando o que ainda não cobre (ofuscação deliberada tipo `eval $(base64 -d ...)` — fechar isso de vez significa escrever um parser de shell, desproporcional pra um backstop opt-in e desligado por padrão). Perguntado "faltou algo?" depois, uma auto-auditoria achou mais 2 gaps: `--contract` nunca chegava nas tasks filhas sob `/loop --parallel` (a flag não estava na lista explícita de propagação que `buildChildArgs()` usa), e nem `contract.mjs` nem o guard tinham teste automatizado — só validação manual via bash, que não vira checagem de regressão. Os dois corrigidos: a flag agora propaga igual ao `--stop-when`, e dois arquivos de teste novos (`test-contract.mjs`, `test-permission-ladder-guard.mjs`) levam a suíte a 23 arquivos, incluindo os casos exatos de bypass que o review de segurança achou. | [`hooks/scripts/permission-ladder-guard.mjs`](hooks/scripts/permission-ladder-guard.mjs), [`scripts/auto-loop/parallel.mjs`](scripts/auto-loop/parallel.mjs), [`scripts/tests/auto-loop/test-contract.mjs`](scripts/tests/auto-loop/test-contract.mjs), [`scripts/tests/auto-loop/test-permission-ladder-guard.mjs`](scripts/tests/auto-loop/test-permission-ladder-guard.mjs) |
| **não lançado** | **Follow-up de enforcement: 3 dos 5 gaps acima agora têm mecanismo real por trás, não só prosa de policy.** Uma auditoria contra o código real (não só a doc) encontrou que o commit anterior tinha documentado task contract, permission ladder e taxonomia de destino-de-fix só como prosa — nenhum schema, nenhum hook, nenhum código lendo nada disso. Isso fecha o gap: `/loop --contract <path.json>` (flag nova) carrega um contrato JSON (`schemas/task-contract.schema.json`) e injeta `done_when`/`escalate_when` no prompt de cada iteração — `escalate_when` tem prioridade sobre as heurísticas de conclusão e sai com código dedicado (8) em vez de deixar o agente inventar um jeito de seguir sozinho. `hooks/scripts/permission-ladder-guard.mjs` (novo, **desligado por padrão**) é um check PreToolUse pro degrau inequivocamente alto da régua — delete recursivo/forçado, `git push --force` sem `--force-with-lease`, `git reset --hard`, `git branch -D`, comandos com cara de deploy/publish — pedindo aprovação com a evidência que a policy diz que precisa acompanhar. O `status.json` por run do `scripts/auto-loop/runner.mjs` ganhou campos de trace (`stopReason`, `rollbackPoint`, `fixDestination`, `contractVersion`) de forma puramente aditiva, verificado contra a suíte de 21 testes já existente e o único consumidor (`parallel.mjs`) que já lê esse arquivo. A métrica (`accepted outputs / human review minutes`) continua só documentação — precisa de dado de tempo humano de revisão que nenhum hook do kit consegue observar. | [`scripts/auto-loop/contract.mjs`](scripts/auto-loop/contract.mjs), [`hooks/scripts/permission-ladder-guard.mjs`](hooks/scripts/permission-ladder-guard.mjs), [`scripts/auto-loop/runner.mjs`](scripts/auto-loop/runner.mjs) |
| **não lançado** | **Cinco gaps absorvidos de uma segunda peça de [Harness Engineering](https://x.com/iiiichigo_chan/status/2093765205276713218) da Birgitta Böckeler, depois de um subagent de investigação comparar as seis camadas do artigo contra o kit inteiro antes de qualquer implementação.** A maior parte do framework do artigo (context compiler, durable state em 4 tiers, evidence gates maker/checker) já existia sob outro nome — só os gaps genuínos viraram edição: um schema formal de task contract (`contract_version`, `done_when`/`escalate_when` como campos estruturados) pra trabalho que atravessa pipeline/loop/handoff; uma resposta estruturada de tool (`status`/`reason`/`evidence`/`retryable`) mais uma permission ladder nomeada mapeando tipo de ação pro que precisa acompanhar a aprovação (diff revisável, preview de conteúdo, plano de rollback, plano de recuperação); uma taxonomia de falha por destino-de-fix (`missing_context`/`bad_tool_contract`/`missing_guardrail`/`weak_verification`) que complementa — não substitui — a taxonomia por camada já existente; e uma métrica-alvo, `accepted outputs / human review minutes`, preenchendo um ponto cego que o relatório de savings já nomeava mas nunca media. | [`policies/goal-driven-execution.md`](policies/goal-driven-execution.md), [`policies/tool-safety.md`](policies/tool-safety.md), [`agents/debugger.md`](agents/debugger.md), [`policies/savings-metrics.md`](policies/savings-metrics.md) |

...
# Dev Team Kit — 76 Skills Especialistas para Coding Agents
...
Backend, frontend, mobile (Tauri), observability, analytics, acessibilidade (WCAG), refatoração, release, documentação — **76 especialistas no total** (76 diretórios de skill instalados; o ID 16 é reservado). Cada task vai pro profissional certo, com o modelo de IA certo (Haiku pro simples, Sonnet pro médio, Opus pra arquitetura) — você não paga Opus pra gerar boilerplate.
...
O **Dev Team Kit** é um conjunto de 76 skills especializadas que transforma qualquer agente de coding compatível em um time completo de desenvolvimento — com orchestrator, backend, frontend, QA, security, deploy, design, copy, SEO, observability e mais.
...
Instala as 76 skills e hooks globalmente. Funciona em qualquer projeto sem configuração adicional.
...
Na tabela abaixo, considere o `dev-team-kit` como 38 tools apoiadas pelas 76 skills (76 diretórios de skill instalados; o ID 16 é reservado).
...
| 10 | **Documenter** | registra decisões, contratos, operações, Repo-Wikis, trilhas funcionais e HTML offline pesquisável |
...
| 35 | **Skill Author** | meta-skill para criar, editar, avaliar e otimizar as próprias skills do kit — sustenta o kit conforme cresce além de 76 especialistas |
...
Ao rodar a skill Documenter dentro de um projeto, o Repo-Wiki usa a pasta corrente como alvo e gera, por padrão, `docs/repo-wiki/` nesse mesmo projeto. A saída inclui Markdown, relatório de cobertura e um site HTML offline navegável:
...
| 76 | **Diagram Validated** | gera SVG determinístico para 14 tipos UML + arquitetura/fluxo em 12 estilos nomeados, valida geometria (seta órfã, rótulo sobreposto, nó fora do canvas), e faz readback visual em PNG antes de declarar pronto. "Avalie, não afirme" — conclusão sustentada por validador + evidência de render, não pelo julgamento visual do próprio modelo |
| 77 | **Frontend Slides** | decks HTML single-file em palco fixo 1920×1080 que escala uniformemente (letterbox, nunca reflow) — 12 presets visuais nomeados, fase de descoberta com 3 previews (mostra, não descreve) antes de construir o deck completo, import de PPTX, export em PDF, deploy no Vercel |
...
| [sopaco/deepwiki-rs](https://github.com/sopaco/deepwiki-rs) | Inspirou o modo Repo-Wiki do Documenter: análise em etapas, composição C4, pesquisa por domínio, chunking de conhecimento local, proveniência, cache e relatório de integridade | v2.77.0 |

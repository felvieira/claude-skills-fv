...
import { summarizeRouteFeedbackSync } from "./lib/route-feedback.mjs";
...
function collectRoutingFeedback(root, sinceMs) {
  try {
    return summarizeRouteFeedbackSync({ root, sinceMs });
  } catch {
    return {
      total: 0,
      by_decision: { accepted: 0, overridden: 0, rejected: 0 },
      acceptance_rate: null,
      overrides_or_rejections: 0,
      top_plugins: [],
    };
  }
}
...
function collectHarnessCoverage(root, sinceMs) {
  ...
  const sensors = {
    ...
    "route-feedback":           { source: ".bot/route-feedback.jsonl",       category: "meta" },
...
function aggregate({ root, since }) {
  ...
  const routing = collectRoutingFeedback(root, sinceMs);
...
function renderMini(report) {
  ...
  if (report.routing.total > 0) {
    lines.push(`  • routing: ${report.routing.by_decision.accepted} accepted, ${report.routing.overrides_or_rejections} overridden/rejected`);
  }
...
function renderMarkdown(report) {
  ...
  md.push(`## Routing Feedback`);
  md.push(``);
  if (report.routing.total === 0) {
    md.push(`No routing decisions recorded yet. After a routed run, record \`accepted\`, \`overridden\`, or \`rejected\` with \`scripts/route-feedback.mjs\` before changing catalog triggers.`);
  } else {
    const rate = report.routing.acceptance_rate === null ? "n/a" : `${(report.routing.acceptance_rate * 100).toFixed(1)}%`;
    md.push(`- **Decisions:** ${report.routing.total}`);
    md.push(`- **Accepted:** ${report.routing.by_decision.accepted}; **overridden:** ${report.routing.by_decision.overridden}; **rejected:** ${report.routing.by_decision.rejected}`);
    md.push(`- **Acceptance rate:** ${rate}`);
    if (report.routing.overrides_or_rejections / report.routing.total >= 0.3) {
      md.push(`- Overrides/rejections are at least 30%; review catalog triggers before adding skills.`);
    }
    if (report.routing.top_plugins.length) {
      md.push(`- **Top selected plugins:** ${report.routing.top_plugins.map((plugin) => `${plugin.plugin} x${plugin.total}`).join(", ")}`);
    }
  }
  md.push(``);
  md.push(`---`);

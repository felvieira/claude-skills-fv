...
# -*- coding: utf-8 -*-
"""
BM25 search engine for the UI/UX design-decision catalogs (data/*.csv).

Ported near-verbatim from nextlevelbuilder/ui-ux-pro-max-skill (MIT,
https://github.com/nextlevelbuilder/ui-ux-pro-max-skill) -- stdlib-only BM25
implementation with per-domain score calibration, exact-identity resolution,
and deterministic tie-breaking. Kept as-is because it's already tested and
dependency-free; the only trim from upstream is search.py (this file's CLI
wrapper), which drops --design-system/--persist/--variance/--motion/--density
(those generate a full design system and write files -- out of scope here,
skill 02 already owns that decision-making in prose). Data comes from the
same upstream repo, downloaded as one clean copy into ../data/.
"""
